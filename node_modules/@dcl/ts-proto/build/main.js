    ...
    if (options.outputEncodeMethods || options.outputJsonMethods || options.outputTypeRegistry || options.nestJs) {
        ...
        (0, visit_1.visit)(fileDesc, sourceInfo, (fullName, message, _sInfo, fullProtoTypeName) => {
            ...
            if (options.outputTypeRegistry) {
                staticMembers.push((0, ts_poet_1.code) `export const $type = '${fullTypeName}'`);
            }
            if (options.outputExtensions) {
                for (const extension of message.extension) {
                    ...
                    staticMembers.push((0, ts_poet_1.code) `export const ${name}: ${ctx.utils.Extension}<${type}> = ${extensionInfo}`);
                }
            }
            ...
            export namespace ${(0, ts_poet_1.def)(fullName)} {
...
function generateInterfaceDeclaration(ctx, fullName, messageDesc, sourceInfo, fullTypeName) {
    ...
    if (ctx.options.unknownFields) {
        chunks.push((0, ts_poet_1.code) `_unknownFields?: {[key: number]: Uint8Array[]},`);
    }
    chunks.push((0, ts_poet_1.code) `}`);
  ...
  `);
    ...
    `;
        ...
        if ((0, types_1.isRepeated)(field)) {
            const maybeNonNullAssertion = ctx.options.useOptionals === "all" || ctx.options.useOptionals === "repeated_and_oneof" ? "!" : "";
...
function generateFromPartial(ctx, fullName, messageDesc) {
  ...
  `);
    ...
    if (ctx.options.useExactTypes) {
        chunks.push((0, ts_poet_1.code) `
      export function fromPartial<I extends ${utils.Exact}<${utils.DeepPartial}<${fullName}>, I>>(${paramName}: I): ${fullName} {
    `);
    }
    else {
        chunks.push((0, ts_poet_1.code) `
      export function fromPartial(${paramName}: ${utils.DeepPartial}<${fullName}>): ${fullName} {
    `);
    }
    let createBase = (0, ts_poet_1.code) `createBase${fullName}()`;
    if (options.usePrototypeForDefaults) {
